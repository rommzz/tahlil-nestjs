...
export class IuranService {
  constructor(
    ...
    @InjectModel('PeriodeJamaah')
    private periodeJamaahModel: Model<IPeriodeJamaah>,
  ) {}
  async create(createIuranDto: CreateIuranDto) {
    ...
    try {
      ...
      const locationId = await this.jamaahModel.findById(
        createIuranDto.locationId,
      );

      if (locationId == null) {
        throw new NotFoundException(
          `jamaah ${createIuranDto.locationId} tidak ditemukan`,
        );
      }
      ...
      const iuranItem = await this.createIuranItemInstance(
        createIuranDto,
        periode._id,
        iuran._id,
      );
      ...
      if (isNewPeriod) {
        periodeJamaah.forEach((item) => {
          ...
          if (
            item.jamaahId.valueOf() == locationId._id.valueOf() &&
            item.periodId.valueOf() == periode._id.valueOf()
          ) {
  ...
  async createIuranItemInstance(
    ...
    try {
      ...
      createIuranDto.iuranItem.forEach(async (item) => {
        ...
        if (j == null) {
          throw new NotFoundException(
            `jamaah ${item.jamaahId} tidak ditemukan`,
          );
        }
        iuranItem.push(
          new this.iuranItemModel({
        ...
        jamaah.splice(
          jamaah.findIndex((jamaah) => jamaah._id == item.jamaahId),
          1,
        );
      });
      jamaah.forEach((item) => {
